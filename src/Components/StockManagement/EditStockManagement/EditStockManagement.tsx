...
import { ArrowLeft, PackageSearch, Plus, Save, Search, Trash2, X } from "lucide-react";
...
/**
 * Converts stored transaction variants into editable form rows.
 */
const mapTransactionVariantToRow = (variant: StockTransaction["variants"][number]): VariantRow => ({
  id: variant.id,
  size: variant.size,
  color: variant.color,
...
export const EditStockManagement = () => {
  const { variantId } = useParams();
  const { state } = useLocation();
  const productDropdownRef = useRef<HTMLDivElement>(null);

  // Prefer the transaction passed from navigation state, then fall back to the mock list by route ID.
  const initialTransaction = useMemo(() => {
    const transactionFromState = (state as { transaction?: StockTransaction })?.transaction;
    return transactionFromState ?? MOCK_STOCK_TRANSACTIONS.find((transaction) => transaction.id === variantId) ?? null;
  }, [state, variantId]);

  const [selectedProductId, setSelectedProductId] = useState(initialTransaction?.productId ?? "");
  const [productSearch, setProductSearch] = useState("");
  const [isProductMenuOpen, setIsProductMenuOpen] = useState(false);
  const [price, setPrice] = useState(initialTransaction?.price?.toString() ?? "");
  const [transactionType, setTransactionType] = useState<TransactionType>(initialTransaction?.type ?? "Shipping");
  const [note, setNote] = useState(initialTransaction?.note ?? "");
  const [variantRows, setVariantRows] = useState<VariantRow[]>(
    initialTransaction?.variants.length
      ? initialTransaction.variants.map(mapTransactionVariantToRow)
      : [createEmptyRow()],
  );
  ...
  const selectedProduct = useMemo(() => {
    return INVENTORY_PRODUCTS.find((product) => product.id === selectedProductId) ?? null
  }, [selectedProductId],
  );

  // Keep the searchable input synced with the selected product when loading an existing transaction.
  useEffect(() => {
    if (selectedProduct) {
  ...
  // Sets the selected product, updates search text, resets variants, and closes the dropdown menu.
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    const payload = {
      id: initialTransaction?.id ?? variantId,
    ...
    console.log("Update stock transaction payload:", payload);
    setErrorMessage("");
  };
    ...
    return (
      <div className="min-h-screen">
        <div className="mx-auto max-w-6xl p-6 shadow-sm ring-1 ring-slate-200/70">
          <div className="rounded-[10px] border border-slate-200 bg-white p-8 text-center">
            <h1 className="text-2xl font-bold text-slate-900">Transaction not found</h1>
            <p className="mt-3 text-sm text-slate-500">
              The selected transaction could not be loaded for editing.
            </p>
            <Link
              to="/admin/stock-management"
              className="mt-6 inline-flex items-center gap-2 rounded-[10px] bg-(--main-color) px-5 py-3 text-sm font-semibold text-white transition hover:bg-(--hover-color)"
            >
              <ArrowLeft size={16} />
      ...
      <div className="mx-auto max-w-6xl p-6 shadow-sm ring-1 ring-slate-200/70">
        <div className="mb-8 flex flex-col gap-4 border-b border-slate-200 pb-6 md:flex-row md:items-center md:justify-between">
          <div className="space-y-3">
            ...
            <div>
              <h1 className="text-3xl font-bold tracking-tight text-slate-950">Edit Transaction</h1>
            </div>
          </div>

          <button
            type="submit"
            form="edit-stock-transaction-form"
        ...
        <form id="edit-stock-transaction-form" onSubmit={handleSubmit} className="space-y-8">
              ...
              <div className={`grid gap-5 ${isPriceVisible ? "md:grid-cols-2 xl:grid-cols-[1.25fr_1fr_1fr]" : "md:grid-cols-2 xl:grid-cols-[1.4fr_1fr]"}`}>
                <Field label="Product">
                  <div ref={productDropdownRef} className="relative">
                    <div className="flex h-14 items-center rounded-[10px] border border-slate-200 bg-white px-4 py-4 text-slate-900 transition focus-within:border-(--main-color) focus-within:ring-4 focus-within:ring-[#d9f1ee]">
              ...
              <section className="rounded-[10px] border border-slate-200 bg-linear-to-r from-[#003334] to-[#014849d7] p-6 text-white shadow-sm">
                <div className="space-y-2">
                  ...
                  <p className="text-sm text-slate-200">
                    Update the context so the team understands why this stock movement changed.
                  </p>
                </div>
                ...
                {errorMessage ? (
                  ...
                  <div className="mt-4 rounded-[10px] border border-white/15 bg-white/10 px-4 py-3 text-sm text-slate-100">
                    Review the prefilled transaction and update any fields before saving.
                  </div>
                )}
              </section>
            </div>
